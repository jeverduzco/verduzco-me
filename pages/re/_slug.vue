...
export default {
  ...
  beforeMount() {
    if (this.$route.params.slug) {
      switch (this.$route.params.slug) {
        case 'tw-to-live':
          this.$appInsights.trackEvent({ name: 'go-to-twitch-from-twitter' })
          window.location.href = this.targets.twitch
          break
        case 'li-to-live':
          this.$appInsights.trackEvent({ name: 'go-to-twitch-from-linkedin' })
          window.location.href = this.targets.twitch
          break
        case 'fb-to-live':
          this.$appInsights.trackEvent({ name: 'go-to-twitch-from-facebook' })
          window.location.href = this.targets.twitch
          break
        default:
          this.$router.push(this.$i18n.locale === 'en' ? '/en/' : '/es/')
      }
    } else {
      this.$router.push(this.$i18n.locale === 'en' ? '/en/' : '/es/')
    }
  ...
  head() {
    return {
      ...
      title:
        this.$i18n.locale === 'en' ? 'Redirecting...' : 'Redireccionando...',
      link: [
        {
          rel: 'canonical',
          href:
